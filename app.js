...
const calculateDivergence = (temp, humidity, iaq, lux, sound) => {
  ...
  return 0.3 * fT + 0.2 * fH + 0.25 * fG + 0.1 * fL + 0.15 * fS;
};
...
const getDivergenceCategory = (d) => {
  if (d >= 0.9) return { label: 'PERFECT', color: '#00ff88', emoji: '🟢' };
  if (d >= 0.8) return { label: 'EXCELLENT', color: '#88ff88', emoji: '🟢' };
  if (d >= 0.6) return { label: 'ACCEPTABLE', color: '#ffcc66', emoji: '🟡' };
  if (d >= 0.2) return { label: 'POOR', color: '#ff9944', emoji: '🟠' };
...
function LiveDataMode() {
  ...
  // ESP32 connection and data fetch
  const fetchLiveData = async () => {
    try {
      setRefreshing(true);

      const res = await fetch('http://192.168.0.202/data'); // JSON ENDPOINT
...
function SimulationMode() {
    ...
    const fetchCityData = async (cityName) => {
      try {
        ...
        const buildingQuality =
          BUILDING_QUALITY[countryCode] ?? BUILDING_QUALITY.DEFAULT;
        ...
        const baseIAQ =
          outdoorAQI * infiltrationFactor * 1.5 + 20 + Math.random() * 30;
        ...
        const outdoorHumidity =
          weatherData.hourly.relativehumidity_2m[currentHour];
        ...
        let indoorSound =
          noiseBase + (5 + Math.random() * 15) * (1 - buildingQuality * 0.3);
  ...
  return (
    <View>
      ...
      {inputMode === 'manual' && (
        <View>
          ...
          <InputRow
            ...
            onChangeText={(v) =>
              setManualData({ ...manualData, temperature: v })
            }
            placeholder="23.5"
          />
          <InputRow
          ...
          <InputRow
            ...
            onChangeText={(v) => setManualData({ ...manualData, iaq: v })}
            placeholder="35"
          />
          <InputRow
            label="LUMINOSITY (LUX)"
            value={manualData.lux}
            onChangeText={(v) => setManualData({ ...manualData, lux: v })}
            placeholder="400"
          />
          <InputRow
            label="SOUND (dB)"
            value={manualData.sound}
            onChangeText={(v) => setManualData({ ...manualData, sound: v })}
            placeholder="35"
          />
        </View>
      )}
          ...
          {loading ? (
            ...
            <>
              ...
              <Row
                ...
                label="HUMIDITY"
                value={
                  apiData.humidity !== null ? `${apiData.humidity}%` : 'N/A'
                }
              />
              <Row
                icon={<AqiIcon />}
      ...
      <Text style={styles.text}>
        {'   '}Temperature:{' '}
        {normalizeTemperature(parseFloat(currentData.temperature) || 0).toFixed(
          3
        )}{' '}
        (30% weight)
      </Text>
      <Text style={styles.text}>
        {'   '}Humidity:{' '}
        {normalizeHumidity(parseFloat(currentData.humidity) || 0).toFixed(3)}{' '}
        (20% weight)
      </Text>
      <Text style={styles.text}>
        {'   '}IAQ: {normalizeIAQ(parseFloat(currentData.iaq) || 0).toFixed(3)}{' '}
        (25% weight)
      </Text>
      <Text style={styles.text}>
        {'   '}Light:{' '}
        {normalizeLux(parseFloat(currentData.lux) || 0).toFixed(3)} (10% weight)
      </Text>
      <Text style={styles.text}>
        {'   '}Sound:{' '}
        {normalizeSound(parseFloat(currentData.sound) || 0).toFixed(3)} (15%
        weight)
      </Text>
    </View>
  );
...
function DashboardMode() {
  ...
  if (latest?.gas > 120)
    recommendations.push('• Improve ventilation (IAQ high)');
  if (latest?.light < 200) recommendations.push('• Increase lighting');
  if (latest?.sound > 60) recommendations.push('• Reduce noise levels');
  if (latest?.humidity > 65) recommendations.push('• Reduce humidity');
  if (recommendations.length === 0) {
    recommendations.push('• Environment is stable');
  }
  ...
  const alerts =
    dbData.length === 0
      ...
      : dbData
          .slice(0, 10)
          .filter((d) => d.divergence < 0.5 || d.gas > 150)
          .map((d) => ({
            icon: d.divergence < 0.3 ? '🔴' : '⚠️',
            msg: `D=${d.divergence.toFixed(2)} | IAQ=${d.gas}`,
          }));

  if (alerts.length === 0) {
    alerts.push({ icon: '🟢', msg: 'All conditions normal' });
  }

  // Trend

  let trend = { direction: '-', label: 'STABLE', detail: 'Not enough data' };

  if (dbData.length >= 2) {
    const latest = dbData[0].divergence;
    const prev = dbData[1].divergence;

    if (latest > prev) {
      trend = {
        direction: '↑',
        label: 'IMPROVING',
        detail: 'Divergence increasing',
      };
    } else if (latest < prev) {
      trend = {
        direction: '↓',
  ...
  const extremes = {
    highest:
      dbData.length > 0
        ? dbData.reduce((max, d) => (d.divergence > max.divergence ? d : max))
        : null,

    lowest:
      dbData.length > 0
        ? dbData.reduce((min, d) => (d.divergence < min.divergence ? d : min))
        : null,
  };

  // Distribution

  const distribution = dbData.reduce(
    (acc, d) => {
      const cat = getDivergenceCategory(d.divergence).label;

      if (cat === 'PERFECT' || cat === 'EXCELLENT') acc.good++;
      else if (cat === 'ACCEPTABLE') acc.ok++;
      else if (cat === 'POOR') acc.poor++;
      else acc.severe++;

      return acc;
    },
    { good: 0, ok: 0, poor: 0, severe: 0 }
  );

  const overview = {
    current: latest?.divergence ?? 0,
    worldLine: latest ? getWorldLine(latest.divergence) : 'WAITING',
    avg:
      dbData.length > 0
        ? dbData.reduce((sum, d) => sum + d.divergence, 0) / dbData.length
        : 0,
    best: dbData.length > 0 ? Math.max(...dbData.map((d) => d.divergence)) : 0,
    worst: dbData.length > 0 ? Math.min(...dbData.map((d) => d.divergence)) : 0,
  };
  ...
  dbData.forEach((d) => {
    ...
    if (!dailySummaryMap[day]) {
      dailySummaryMap[day] = [];
    }

    dailySummaryMap[day].push(d.divergence);
  });

  const dailySummary = Object.entries(dailySummaryMap)
  ...
  const getFilteredData = () => {
    if (activeFilter === '24H') return dbData.slice(0, 10);
    if (activeFilter === '7D') return dbData.slice(0, 30);
    if (activeFilter === '14D') return dbData.slice(0, 50);
    return dbData;
  };
  ...
  const safeDistribution =
    total === 0 ? { good: 1, ok: 0, poor: 0, severe: 0 } : distribution;

  const pieChartData = [
    {
      name: 'GOOD',
      population: safeDistribution.good,
      color: '#00ff88',
      legendFontColor: '#00ff88',
      legendFontSize: 11,
    },
    {
      name: 'OK',
      population: safeDistribution.ok,
      color: '#ffcc66',
      legendFontColor: '#ffcc66',
      legendFontSize: 11,
    },
    {
      name: 'POOR',
      population: safeDistribution.poor,
      color: '#ff9944',
      legendFontColor: '#ff9944',
      legendFontSize: 11,
    },
    {
      name: 'SEVERE',
      population: safeDistribution.severe,
      color: '#ff4444',
      legendFontColor: '#ff4444',
      legendFontSize: 11,
    },
  ];
  ...
  if (loading) {
    return (
      <View>
        <Line text="LOADING DASHBOARD DATA..." />
      </View>
    );
  }
  ...
  return (
    <View>
      ...
      <Row
        icon={<DivergenceIcon />}
        label="CURRENT"
        value={overview.current?.toFixed?.(6) ?? '0.000000'}
        color={valColor(overview.current)}
      />
      <Line text="[SENSOR SNAPSHOT]" />
      <Spacer />
      ...
      <Row label="IAQ" value={latest?.gas != null ? `${latest.gas}` : '--'} />

      <Row
        label="LIGHT"
        value={latest?.light != null ? `${latest.light} LUX` : '--'}
      />

      <Row
        label="SOUND"
        value={latest?.sound != null ? `${latest.sound} dB` : '--'}
      />
      ...
      <Row
        icon={<Text style={{ fontSize: 16, color: '#fff' }}>🌐</Text>}
        label="WORLD LINE"
        value={overview.worldLine}
        color={getWorldLineColor(overview.worldLine)}
      />
      <Row
        icon={<Text style={{ fontSize: 14, color: '#00ff88' }}>~</Text>}
        label="TODAY AVG"
        value={overview.avg?.toFixed?.(6) ?? '0.000000'}
        color={valColor(overview.avg)}
      />
      <Row
        icon={<Text style={{ fontSize: 14, color: '#00ff88' }}>↑</Text>}
        label="BEST"
        value={overview.best?.toFixed?.(6) ?? '0.000000'}
        color={valColor(overview.best)}
      />
      <Row
        icon={<Text style={{ fontSize: 14, color: '#00ff88' }}>↓</Text>}
        label="WORST"
        value={overview.worst?.toFixed?.(6) ?? '0.000000'}
        color={valColor(overview.worst)}
      />
      <Spacer />
      ...
      <Text style={[styles.text, { color: '#00ff88' }]}>
        {'   '}HIGHEST : {extremes.highest?.divergence?.toFixed(6) ?? '--'}
      </Text>

      <Text style={[styles.text, { color: '#ff4444' }]}>
        {'   '}LOWEST : {extremes.lowest?.divergence?.toFixed(6) ?? '--'}
      </Text>
      <Spacer />
...
function Row({ icon, label, value, color }) {
  return (
    <View style={[styles.row, { flexWrap: 'wrap' }]}>
      {icon}
      <Text style={[styles.text, color && { color }, { flexShrink: 1 }]}>
        {' '}
        {label} : {value}
      </Text>
    </View>
